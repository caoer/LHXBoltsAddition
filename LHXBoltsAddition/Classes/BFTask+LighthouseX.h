...
- (BFTask *)continueWithMainThreadBooleanResultBlock:(void (^)(BOOL succeeded, NSError *_Nullable error))resultBlock
                                  executeIfCancelled:(BOOL)executeIfCancelled;
...
/**
 pure side effect, log error if there is any. 

 @return same task as passed in
 */
- (BFTask *)continueWithErrorLogging;
@end


@interface BFTask<__covariant ResultType> (BoltsExtras)
