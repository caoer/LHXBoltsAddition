...
BOOL PSPDFReplaceMethodWithBlock(Class c, SEL origSEL, SEL newSEL, id block) {
    ...
    if (!class_addMethod(c, newSEL, impl, method_getTypeEncoding(origMethod))) {
        LHXLogError(@"Failed to add method: %@ on %@", NSStringFromSelector(newSEL), c);
        return NO;
    }else {
...
//    PSPDFAssert(NSThread.isMainThread, @"\nERROR: All calls to UIKit need to happen on the main thread. You have a bug in your code. Use dispatch_async(dispatch_get_main_queue(), ^{ ... }); if you're unsure what thread you're in.\n\nBreak on PSPDFAssertIfNotMainThread to find out where.\n\nStacktrace: %@", NSThread.callStackSymbols);
    if (![NSThread isMainThread]) {
        LHXLogError(@"\nError: call thread error, should not call UIKit method in non mainthread:\n%@", [NSThread callStackSymbols]);
    }
}
...
//        NSString *selectorString = @"layoutSublayers";
//        SEL selector = NSSelectorFromString(selectorString);
//        SEL newSelector = NSSelectorFromString([NSString stringWithFormat:@"pspdf_%@", selectorString]);
//        
//        PSPDFReplaceMethodWithBlock(CALayer.class, selector, newSelector, ^(__unsafe_unretained CALayer *_self) {
//            if (_self.delegate) {
//                if (![NSThread isMainThread]) {
//#pragma clang diagnostic push
//#pragma clang diagnostic ignored "-Wdeprecated-declarations"
