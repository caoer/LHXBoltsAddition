...
void LHXForceLoadUIColorLHXColor() {}

@implementation UIColor (LHXColors)
...
{
    return [UIColor lhx_colorWithHue:hue saturation:saturation lightness:lightness alpha:1.0];
}

+ (UIColor *)lhx_colorWithHue:(CGFloat)hue saturation:(CGFloat)saturation lightness:(CGFloat)lightness alpha:(CGFloat)alpha
{
    CGFloat hh = hue;
    CGFloat ss = saturation;
    CGFloat ll = lightness;
...
{
    return [UIColor lhx_colorWithHexString:hexString withAlpha:1.0];
}
...
+ (UIColor *)lhx_colorWithHexString:(NSString *)hexString withAlpha:(CGFloat)alpha
{
    ...
    if (hexString)
    {
        NSUInteger startingIndex = ([hexString hasPrefix:@"#"]) ? 1 : 0;
    ...
    return [UIColor lhx_colorWithARGBValue:value];
}
    ...
    if ([self getRed:&r green:&g blue:&b alpha:alpha])
    {
        ...
        if (l > 0.0f)
        {
            ...
            if (s > 0.0f)
            {
                ...
                CGFloat r2 = (max - r) / range;
                CGFloat g2 = (max - g) / range;
                CGFloat b2 = (max - b) / range;
...
- (UIColor *)lhx_lightenByPercent:(CGFloat)percent
{
    ...
    [self lhx_getHue:&hue saturation:&saturation lightness:&lightness alpha:&alpha];

    lightness = MIN(1.0f, lightness + percent * 0.01f);

    return [UIColor lhx_colorWithHue:hue saturation:saturation lightness:lightness alpha:alpha];
}
